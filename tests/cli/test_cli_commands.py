...
class TestSampleCLICommands(unittest.TestCase):
    ...
    def test_run_sample(self):
        output = subprocess.run(["python", QUBER_CLI, "run-sample"], capture_output = True, text = True)
        with open('./result.yaml', 'r', encoding='utf-8') as file:
            result = yaml.safe_load(file)
            self.assertEqual(19,  result['params']['result'])

    def test_calc_sample(self):
        output = subprocess.run(["python", QUBER_CLI, "calc", "--context_path=context2.yaml"], capture_output = True, text = True)
        with open('./result_calc.yaml', 'r', encoding='utf-8') as file:
            result = yaml.safe_load(file)
